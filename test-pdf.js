...
async function testPDF() {
  const { data, error } = await supabase
    .from('presupuestos')
    .select('id, cliente, subtotal, analisis_json')
    .not('analisis_json', 'is', null)
    .limit(1)
  ...
  if (data && data.length > 0) {
    ...
    console.log('Análisis JSON:', JSON.stringify(presupuesto.analisis_json, null, 2))
  } else {
    console.log('No presupuestos found with análisis')
  }
}
